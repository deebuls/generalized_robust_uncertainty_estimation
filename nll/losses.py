...
def generalized_nll_loss(input, target, alpha, beta, eps=1e-6, reduction='mean'):
  ...
  per_pixel_loss = tf.pow(tf.abs(input - target) / alpha, beta) - tf.math.log(beta) + tf.math.log(2 * alpha) + tf.math.lgamma(1 / beta)
